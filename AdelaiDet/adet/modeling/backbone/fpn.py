...
class DepthWiseConv(nn.Module):
    def __init__(self, in_channel, out_channel, stride_dw):
...
class LastLevelP6P7(nn.Module):    
    ...
    def __init__(self, in_channels, out_channels, in_features="res5"):
        ...
        self.p6 = DepthWiseConv(in_channels, out_channels, 2)
        self.p7 = DepthWiseConv(out_channels, out_channels, 2)
        for module in [self.p6.depth_conv, self.p6.point_conv, self.p7.depth_conv, self.p7.point_conv]:
            weight_init.c2_xavier_fill(module)
...
def build_fcos_resnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    backbone = FPN(
        bottom_up=bottom_up,
        in_features=in_features,  # res3, res4, res5
        out_channels=out_channels,  # 256
        norm=cfg.MODEL.FPN.NORM,
        top_block=top_block,  # p6, p7
